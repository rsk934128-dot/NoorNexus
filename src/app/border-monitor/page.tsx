...
import { Radar, ShieldCheck, Terminal, Menu, Loader2, Activity, RefreshCcw, ShieldHalf, Globe, Zap, AlertTriangle, Database } from "lucide-react"
import { Table, TableBody, TableCell, TableHead, TableHeader, TableRow } from "@/components/ui/table"
import { Button } from "@/components/ui/button"

const INITIAL_TRAFFIC_LOGS = [
  { id: 1, app: "Brave", destination: "go-updater.brave.com", status: "200 (OK)", risk: "None", type: "Update Service" },
  { id: 2, app: "Brave", destination: "64.227.47.137", status: "200 (OK)", risk: "None", type: "Direct IP" },
  { id: 3, app: "Brave", destination: "184.32.113.194", status: "200 (OK)", risk: "None", type: "CDN Node" },
  { id: 4, app: "Brave", destination: "172.65.55.248", status: "200 (OK)", risk: "None", type: "Cloudflare" },
  { id: 5, app: "Brave", destination: "quorum.wdp.brave.com", status: "200 (OK)", risk: "None", type: "Telemetry" },
  { id: 6, app: "Brave", destination: "collector.wdp.brave.com", status: "200 (OK)", risk: "None", type: "Telemetry" },
  { id: 7, app: "Brave", destination: "32.185.153.76", status: "200 (OK)", risk: "None", type: "Direct IP" },
  { id: 8, app: "Brave", destination: "componentupdater.brave.com", status: "200 (OK)", risk: "None", type: "Update Service" },
  { id: 9, app: "Brave", destination: "dns.google", status: "200 (OK)", risk: "None", type: "DNS Request" },
  { id: 10, app: "Brave", destination: "accounts.youtube.com", status: "200 (OK)", risk: "None", type: "Auth" },
  { id: 11, app: "Brave", destination: "brunhild.challenges.cloudflare.com", status: "200 (OK)", risk: "Low", type: "Security Challenge" },
  { id: 12, app: "Unknown App", destination: "142.251.223.161", status: "Shutdown", risk: "Critical", type: "Intercepted" },
]
          ...
          <header className="flex flex-col sm:flex-row sm:items-center justify-between gap-6 border-b border-white/5 pb-10">
            <div className="space-y-1">
              <div className="flex items-center gap-3">
                 ...
                 <h2 className="text-2xl sm:text-4xl font-headline font-bold flex items-center gap-3 uppercase tracking-tighter">
                   <Radar className="size-8 text-primary animate-pulse" />
                   Shield Matrix.
                 </h2>
              </div>
              <p className="text-muted-foreground text-sm sm:text-base italic">"Monitoring the Digital Border. নূরনেক্সাস এখন প্রতিটি সিগন্যাল মনিটর করছে।"</p>
            </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
            <Card className="glass-card lg:col-span-3 overflow-hidden border-t-4 border-t-primary">
              <CardHeader className="border-b border-white/5 bg-white/2 flex flex-row items-center justify-between py-4 px-6">
                ...
                <Badge variant="outline" className="border-emerald-500/20 text-emerald-500 uppercase font-bold text-[8px]">Hegemony_Scan: ACTIVE</Badge>
              </CardHeader>
                      ...
                      <TableBody>
                        {logs.map((log) => (
                          <TableRow key={log.id} className={`border-white/5 transition-colors ${log.risk === 'Critical' ? 'bg-destructive/10' : 'hover:bg-white/5'}`}>
                            ...
                            <TableCell>
                              <Badge variant="outline" className={`text-[8px] uppercase font-bold ${log.status === '200 (OK)' ? 'border-emerald-500/30 text-emerald-500' : 'border-destructive/30 text-destructive'}`}>
                                {log.status}
                              </Badge>
                            </TableCell>
                ...
                <CardContent className="space-y-6">
                  ...
                  <div className="space-y-3">
                     {[
                       ...
                       { label: "Brave Telemetry", val: "Monitored", color: "text-amber-500" }
                     ].map((item, i) => (
              ...
              <Card className="glass-card border-emerald-500/20 bg-emerald-500/5">
                <CardHeader>
                   <CardTitle className="text-xs uppercase font-bold text-emerald-500 tracking-widest flex items-center gap-2">
                     <Globe className="size-4" /> Global Intelligence
                   </CardTitle>
                </CardHeader>
                <CardContent className="space-y-4">
                   <p className="text-[10px] text-muted-foreground leading-relaxed italic">
                     "কমান্ডার, Brave Browser-এর নতুন ট্রাফিক লগগুলো বিশ্লেষণ করা হয়েছে। এর মাধ্যমে ব্রাউজারের নিয়মিত আপডেট এবং টেলিমেট্রি সার্ভিস শনাক্ত হয়েছে।"
                   </p>
                   <Button variant="outline" className="w-full h-9 text-[9px] uppercase font-bold border-emerald-500/20 text-emerald-500 hover:bg-emerald-500/10">
              ...
              <Card className="glass-card border-white/5">
                  <CardHeader className="pb-2">
                     <CardTitle className="text-[10px] uppercase font-bold text-muted-foreground flex items-center gap-2">
                        <Database className="size-3" /> Grid Memory
                     </CardTitle>
                  </CardHeader>
                  <CardContent>
                     <p className="text-[9px] text-muted-foreground leading-relaxed">
                        Last scan: <strong>Success</strong>. {INITIAL_TRAFFIC_LOGS.length} legitimate handshakes recorded in the last cycle.
                     </p>
                  </CardContent>
               </Card>
            </div>
