...
const MARKET_CATEGORIES = [
  { id: "aerospace", name: "Aerospace & Space", icon: Rocket, color: "text-purple-500", bg: "bg-purple-500/10" },
  { id: "maritime", name: "Maritime & Ships", icon: Ship, color: "text-blue-500", bg: "bg-blue-500/10" },
  { id: "precious", name: "Precious Metals & Gold", icon: Gem, color: "text-amber-500", bg: "bg-amber-500/10" },
  { id: "automotive", name: "Luxury Automotive", icon: Car, color: "text-primary", bg: "bg-primary/10" },
  { id: "tech", name: "High-End Electronics", icon: Smartphone, color: "text-emerald-500", bg: "bg-emerald-500/10" }
]
...
export default function ZenithMarketsPage() {
  ...
  async function handleInitiateTrade(asset: any) {
    ...
    try {
      ...
      // Simulation of a high-value payout/escrow block
      const result = await executeMappedPayout(
        10000, // Partial settlement/escrow fee simulation
        "Sovereign",
          ...
          <section className="flex flex-wrap gap-4">
             ...
             {MARKET_CATEGORIES.map((cat) => (
               <Button 
                 ...
                 className={`text-[10px] uppercase font-bold h-10 px-6 rounded-full gap-2 ${activeCategory === cat.id ? 'bg-amber-500 text-black' : 'border-white/10 hover:border-amber-500/30'}`}
               >
               ...
               <Card className="glass-card border-l-4 border-l-primary bg-primary/5">
                  <CardHeader className="pb-2">
                     <CardTitle className="text-xs font-headline uppercase tracking-widest text-primary flex items-center gap-2">
                        <Scale className="size-4" /> Trade Arbitration Node
                     </CardTitle>
                  </CardHeader>
                  <CardContent className="space-y-4">
                     <p className="text-[9px] text-muted-foreground italic">
                        Every mega-trade is subject to Nora-10 Arbitration in case of dispute, ensuring absolute justice.
                     </p>
                     <Button variant="outline" className="w-full text-[8px] uppercase font-bold border-primary/20 text-primary h-8">
                        View Judicial Rules
                     </Button>
                  </CardContent>
               </Card>
