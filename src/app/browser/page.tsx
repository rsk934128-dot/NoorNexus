...
function BrowserContent() {
  ...
  return (
    <main className="flex flex-col h-screen w-full max-w-full overflow-hidden p-0 m-0 relative">
      <header className="px-3 sm:px-4 py-2 sm:py-3 flex flex-col sm:flex-row items-center gap-3 sm:gap-4 border-b border-white/5 bg-background/50 backdrop-blur-md shrink-0 w-full z-50">
        ...
        <div className="flex-1 flex items-center gap-2 w-full">
           <div className="flex items-center gap-1 bg-white/5 rounded-lg border border-white/10 p-1 shrink-0">
              ...
              <Button variant="ghost" size="icon" className="size-8 text-muted-foreground hover:text-white hidden sm:flex" onClick={handleRefresh}>
           ...
           <div className="flex-1 relative group min-w-0">
              ...
              <Input 
                ...
                className="w-full bg-black/40 border-white/10 h-10 pl-8 pr-12 sm:pr-32 text-[10px] sm:text-xs font-mono text-primary focus:ring-1 focus:ring-primary placeholder:text-muted-foreground/30"
      ...
      <div className="px-3 sm:px-4 py-1.5 sm:py-2 border-b border-white/5 bg-black/20 overflow-x-auto flex items-center gap-3 sm:gap-4 no-scrollbar shrink-0">
         <span className="text-[7px] sm:text-[8px] font-bold text-muted-foreground uppercase tracking-widest shrink-0 flex items-center gap-1.5 sm:gap-2">
           <Radio className="size-2.5 sm:size-3" /> Channels:
         </span>
         {QUICK_LINKS.map((link) => (
           <Button 
            ...
            className="h-7 sm:h-8 text-[7px] sm:text-[9px] uppercase font-bold text-white hover:bg-white/5 gap-1.5 sm:gap-2 px-3 sm:px-4 border border-white/5 rounded-full shrink-0"
           >
             <link.icon className={`size-3 sm:size-3.5 ${link.color}`} />
             {link.name}
           </Button>
         ))}
      </div>
      ...
      <div className="flex-1 overflow-hidden bg-black/40 w-full relative">
        {loading && (
           <div className="absolute inset-0 z-50 bg-background/80 backdrop-blur-xl flex flex-col items-center justify-center gap-6 sm:gap-8 p-4">
              <div className="relative">
                 <div className="size-16 sm:size-24 rounded-full border-2 border-primary/20 flex items-center justify-center">
                    {isSearchMode ? <Cpu className="size-8 sm:size-12 text-amber-500 animate-pulse" /> : <Globe className="size-8 sm:size-12 text-primary animate-pulse" />}
                 </div>
              ...
              <div className="text-center space-y-2 sm:space-y-3">
                 ...
                 <p className="text-[7px] sm:text-[9px] text-muted-foreground font-mono uppercase tracking-[0.2em]">Zenith Veracity Verification: ACTIVE</p>
              </div>
           </div>
        )}
        ...
        {isSearchMode ? (
            ...
            <div className="max-w-5xl mx-auto p-4 sm:p-12 space-y-8 sm:space-y-12">
              {searchResult ? (
                <div className="space-y-8 sm:space-y-12 animate-in fade-in slide-in-from-bottom-6 duration-700">
                  <Card className="glass-card border-l-4 border-l-amber-500 bg-amber-500/5 relative overflow-hidden p-6 sm:p-8">
                    ...
                    <div className="space-y-4 sm:space-y-6 relative z-10">
                       ...
                       <div className="pt-2 sm:pt-4 flex items-center gap-2 sm:gap-3">
                          <Badge className="bg-emerald-500/20 text-emerald-500 border-none text-[7px] sm:text-[8px] px-2 sm:px-3 h-5 sm:h-6 flex items-center gap-1.5 sm:gap-2">
                             <CheckCircle2 className="size-2.5 sm:size-3" /> VERACITY_CONFIRMED
                          </Badge>
                       </div>
                    </div>
                  </Card>
                          ...
                          <Card key={i} className="glass-card border-white/5 hover:border-primary/40 transition-all group cursor-pointer overflow-hidden flex flex-col" onClick={() => handleNavigate(res.url)}>
                            <CardContent className="p-4 sm:p-6 space-y-3 sm:space-y-4 flex-1 flex flex-col">
                               <div className="flex items-start justify-between gap-3 sm:gap-4">
                                  <div className="p-2 sm:p-3 rounded-lg sm:rounded-xl bg-white/5 border border-white/10 group-hover:bg-primary/20 transition-all shrink-0">
                                     <Link2 className="size-4 sm:size-5 text-primary" />
                                  </div>
                                  <div className="min-w-0 flex-1">
                                     <div className="flex items-center gap-2 mb-1">
                                        <Badge variant="outline" className="text-[6px] sm:text-[7px] border-primary/20 text-primary uppercase px-1.5 h-3.5 sm:h-4">{res.source}</Badge>
                                     </div>
                               ...
                               <div className="pt-3 sm:pt-4 mt-auto flex items-center justify-between border-t border-white/5">
                                  ...
                                  <Button variant="ghost" size="icon" className="size-7 sm:size-8 text-primary opacity-0 group-hover:opacity-100 transition-opacity">
                  ...
                  <div className="p-4 sm:p-6 bg-emerald-500/5 border border-emerald-500/20 rounded-xl sm:rounded-2xl flex flex-col sm:flex-row items-center justify-between gap-4 sm:gap-6">
                     <div className="flex items-center gap-3 sm:gap-4 text-center sm:text-left">
                        <div className="size-8 sm:size-10 rounded-full bg-emerald-500/20 flex items-center justify-center border border-emerald-500/30 shrink-0">
                           <Radio className="size-4 sm:size-5 text-emerald-500 animate-pulse" />
                        </div>
                        <div>
                           <p className="text-[8px] sm:text-[10px] font-bold text-emerald-500 uppercase tracking-widest">Imperial Directive</p>
                           <p className="text-[10px] sm:text-xs text-emerald-100 italic leading-snug">"{searchResult.suggestedAction}"</p>
                        </div>
                     </div>
                     <div className="text-center sm:text-right shrink-0">
                        <p className="text-[7px] sm:text-[8px] font-mono text-muted-foreground uppercase mb-0.5 sm:mb-1">Hash (HMAC_V4)</p>
                        <code className="text-[8px] sm:text-[9px] text-primary font-mono">{searchResult.searchHash.substring(0, 15)}...</code>
                     </div>
                  </div>
                </div>
              ) : !loading && (
                <div className="h-[400px] sm:h-[500px] flex flex-col items-center justify-center text-center space-y-6 sm:space-y-8 animate-in fade-in duration-1000">
                   <div className="relative">
                      <div className="size-24 sm:size-32 rounded-full border-4 border-dashed border-primary/20 animate-[spin_10s_linear_infinite]" />
                      <div className="absolute inset-0 flex items-center justify-center">
                         <Search className="size-12 sm:size-16 text-primary opacity-20" />
                      </div>
                   </div>
                   <div className="space-y-2 sm:space-y-2 px-6">
                      <h3 className="text-lg sm:text-xl font-headline font-bold text-white uppercase tracking-widest sm:tracking-[0.2em]">Initiate Search Pulse</h3>
                      <p className="text-[9px] sm:text-xs text-muted-foreground font-mono uppercase tracking-widest leading-relaxed max-w-sm">
                         Commander, enter a query in the terminal above to synthesize web-scale intelligence.
                      </p>
                   </div>
                   <div className="flex flex-wrap justify-center gap-3 sm:gap-4 px-4">
                      <Button onClick={() => handleSearch("Latest Global Economic Outlook")} variant="outline" className="text-[8px] sm:text-[9px] font-bold uppercase border-white/10 h-9 px-4 sm:px-6 hover:border-primary/40">
                         Economic Outlook
                      </Button>
                      <Button onClick={() => handleSearch("Maritime Trade Hubs SE Asia")} variant="outline" className="text-[8px] sm:text-[9px] font-bold uppercase border-white/10 h-9 px-4 sm:px-6 hover:border-primary/40">
                         Maritime Hubs
                      </Button>
                   </div>
                </div>
          ...
          <div className="h-full w-full bg-background flex flex-col items-center justify-center p-4 sm:p-6 space-y-8 sm:space-y-12 animate-in fade-in">
             <div className="max-w-4xl w-full space-y-8 sm:space-y-12">
                <div className="flex flex-col md:flex-row items-center gap-8 sm:gap-12 border-b border-white/5 pb-8 sm:pb-12">
                   <div className="size-20 sm:size-32 rounded-xl sm:rounded-2xl bg-primary/10 border border-primary/20 flex items-center justify-center glow-primary shrink-0">
                      <Infinity className="size-10 sm:size-16 text-primary" />
                   </div>
                   <div className="space-y-3 sm:space-y-4 text-center md:text-left">
                      <h3 className="text-xl sm:text-3xl font-headline font-bold text-white uppercase tracking-tighter">Sovereign Node: {activeUrl.replace('https://', '').replace('.sovereign', '')}</h3>
                      <div className="flex flex-wrap items-center justify-center md:justify-start gap-2 sm:gap-3">
                         <Badge variant="outline" className="border-emerald-500/20 text-emerald-500 uppercase text-[7px] sm:text-[8px] h-5 sm:h-6">Local_Sync: OK</Badge>
                         <Badge variant="outline" className="border-primary/20 text-primary uppercase text-[7px] sm:text-[8px] h-5 sm:h-6">Node_Status: ACTIVE</Badge>
                         <Badge variant="outline" className="border-amber-500/20 text-amber-500 uppercase text-[7px] sm:text-[8px] h-5 sm:h-6">Veracity: 100%</Badge>
                      </div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-4 sm:gap-6">
                   {[
                     ...
                     <Card key={i} className="glass-card bg-white/2 border-white/5">
                        <CardContent className="p-4 sm:p-6 space-y-1 sm:space-y-2 text-center">
                           <item.icon className="size-4 sm:size-6 text-primary mx-auto mb-1.5 sm:mb-2 opacity-50" />
                           <p className="text-[7px] sm:text-[10px] font-bold text-muted-foreground uppercase">{item.label}</p>
                ...
                <Card className="glass-card border-white/5 bg-black/40">
                   <CardHeader className="py-3 sm:py-4 border-b border-white/5">
                      <CardTitle className="text-[10px] sm:text-xs font-headline uppercase text-primary flex items-center gap-2">
                         <Database className="size-3 sm:size-4" /> Node Logs
                      </CardTitle>
                   </CardHeader>
                   <CardContent className="p-4 sm:p-6">
                      <div className="space-y-2 sm:space-y-3">
                         {[
                           ...
                           <div key={i} className="flex justify-between items-center p-2.5 sm:p-3 bg-white/5 rounded-lg sm:rounded-xl border border-white/5 font-mono text-[8px] sm:text-[10px]">
                              <div className="flex items-center gap-2 sm:gap-3">
                                 <span className="text-muted-foreground">[{log.id}]</span>
                                 <span className="text-white uppercase truncate max-w-[120px] sm:max-w-none">{log.action}</span>
                              </div>
             ...
             <div className="flex flex-col items-center gap-3 sm:gap-4 text-center opacity-40">
                <ShieldCheck className="size-8 sm:size-10 text-emerald-500" />
                <p className="text-[7px] sm:text-[8px] font-mono uppercase tracking-[0.3em] sm:tracking-[0.5em]">NoorNexus Internal Only</p>
             </div>
          </div>
        ) : showBypassWarning ? (
          <div className="h-full w-full bg-background flex flex-col items-center justify-center p-6 space-y-8 animate-in fade-in">
             <div className="size-16 sm:size-20 rounded-xl sm:rounded-full bg-amber-500/20 flex items-center justify-center border border-amber-500/40 glow-emerald">
                <ShieldAlert className="size-8 sm:size-10 text-amber-500" />
             </div>
             <div className="text-center space-y-3 sm:space-y-4 max-w-md">
                <h3 className="text-xl sm:text-2xl font-headline font-bold text-white uppercase tracking-tighter">High-Security Node Block</h3>
                <p className="text-[10px] sm:text-xs text-muted-foreground leading-relaxed italic px-4">
             ...
             <div className="flex flex-col sm:flex-row gap-4 w-full sm:w-auto px-6">
                <Button 
                  ...
                  className="w-full sm:w-auto bg-amber-500 text-black font-bold uppercase text-[9px] sm:text-[10px] h-12 px-8 glow-emerald gap-2"
                >
                ...
                <Button 
                  ...
                  className="w-full sm:w-auto border-white/10 text-white font-bold uppercase text-[9px] sm:text-[10px] h-12 px-8"
                >
                   Return to Terminal
                </Button>
             </div>
        ...
        <div className="absolute bottom-4 sm:bottom-6 left-4 sm:left-6 pointer-events-none">
          <div className="p-2 sm:p-3 glass-card rounded-lg flex items-center gap-2 sm:gap-3 border-emerald-500/20 bg-black/60 shadow-2xl">
              <ShieldCheck className="size-3 sm:size-4 text-emerald-500" />
              <div className="space-y-0.5">
                <p className="text-[7px] sm:text-[9px] font-bold text-white uppercase leading-none">Canal Secure</p>
                <p className="text-[6px] sm:text-[7px] text-emerald-500/60 font-mono uppercase leading-none">
      ...
      <footer className="py-2 border-t border-white/5 bg-background/80 shrink-0 text-center w-full z-50">
        <div className="flex items-center justify-center gap-4 sm:gap-8">
           <p className="text-[7px] sm:text-[8px] font-mono text-muted-foreground uppercase tracking-[0.2em] sm:tracking-[0.4em] truncate px-4">
      ...
      <SidebarInset className="flex-1 w-full flex flex-col min-w-0 p-0 m-0">
        <Suspense fallback={
          <div className="h-screen w-full flex flex-col items-center justify-center gap-4 bg-background p-6">
             ...
             <p className="text-[10px] sm:text-xs font-mono uppercase tracking-widest text-primary text-center">Calibrating Terminal...</p>
          </div>
        }>
          <BrowserContent />
        </Suspense>
